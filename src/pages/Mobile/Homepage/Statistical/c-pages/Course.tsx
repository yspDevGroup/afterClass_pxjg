  ...
  const getData = async (res: any) => {
    ...
    if (resProportion.status === 'ok') {
      ...
      if (data) {
        ...
        data.lxs?.length &&
          data.lxs.forEach((item: { KCTAG: any; kc_count: any }) => {
            if (item.kc_count !== 0) {
        ...
        data.xxbm?.length &&
          data.xxbm.forEach((item: { kc_count: any; XXMC: any; bj_count: any }) => {
            if (item.kc_count !== 0) {
  ...
  return (
    <div className={styles.course}>
      ...
      <div className={styles.container} style={{ height: '355px' }}>
        ...
        <div className={styles.chartsContainer}>
          {courseBarConfig.data && courseBarConfig.data?.length !== 0 ? (
            <Bar {...courseBarConfig} />
          ) : (
            <Empty
              image={noData}
